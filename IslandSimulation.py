...
class Island (object):
    ...
    def __init__(self, n, prey_count=0, predator_count=0, human_count=0):
        ...
        '''
        print("Grid Size:",n,"x",n,"Prey:",prey_count,"Predators:",predator_count,"Humans:",human_count)
        self.grid_size = n
        self.grid = []
        for i in range(n):
...
class Human(Animal):
    ...
    def hunt(self):
        ''' Human looks for one of the 8 locations with Predators. If found
        moves to that location, updates the hunt clock, removes the Prey
        '''
        if not self.moved:
            location = self.check_grid(Predator)
            if location and self.hunt_clock == 0:
                # print('Hunting: Human at {},{}, Predator at {},{}'.format( \
                #       self.x,self.y,location[0],location[1]))
                self.island.remove(self.island.animal(location[0],location[1]))
                self.island.remove(self)
                self.x=location[0]
...
###########################################
def main(predator_breed_time=6, predator_starve_time=3, human_breed_time=7, human_starve_time=6,human_hunt_time=8, initial_humans=5, initial_predators=10, prey_breed_time=4, initial_prey=50, \
         size=10, ticks=300):
    ...
    for i in range(ticks):
        ...
        if prey_count == 0:
            ...
            break
        if predator_count == 0:
            print('Lost the Predator population.')
            #break
        if human_count == 0:
            #break
            print("Lost the Human population.")
...
#        ans = input("Return to continue")
    ...
    pylab.plot(range(i), predator_list, label="Predators")
    pylab.plot(range(i), prey_list, label="Prey")
    pylab.plot(range(i), human_list, label="Humans")
